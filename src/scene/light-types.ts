...
export class DirectionalLight implements LightSource {
    constructor(camera: THREE.OrthographicCamera | THREE.PerspectiveCamera, numOfCascades: number) {
        ...
        this.direction = new THREE.Vector3(1.0, 1.0, -1.0).normalize();
        this.splits = [];
        ...
        this.update(camera, numOfCascades);
    }
    ...
    private updateViewProjMatrix(corners: THREE.Vector4[]) {
        const center = corners.reduce(
            (acc: THREE.Vector3, c: THREE.Vector4) => { return acc.add(new THREE.Vector3(c.x, c.y, c.z)); },
            new THREE.Vector3(0, 0, 0)
        ).divideScalar(corners.length);

        this.viewMatrix.lookAt(center, center.clone().add(this.direction), this.upVector);
    ...
    public update(camera: THREE.OrthographicCamera | THREE.PerspectiveCamera, numOfCascades: number) {
        this.setViewProjMatrixSize(numOfCascades);
        
        for(var i = 0; i < numOfCascades; ++i) {
            const zNear = camera.near + camera.far * ((i) / numOfCascades);
            const zFar = camera.near + (camera.far - camera.near) * ((i + 1) / numOfCascades);
            this.splits[i] = {
                near: zNear,
                far: zFar
            };
            ...
            this.updateViewProjMatrix(corners);
